...
library(SQI)
library(mapview)
# Note: this site was built under mapview v 2.7.8. To revert to this version run "devtools::install_version('mapview', '2.7.8')" in the console.
library(leaflet)
library(leafem)
library(sf)
...
# data(sqidat) # calls static dataset of past build
data(sheds)
data(cntys)
data(rwqbs)
data(cnstr)
...
# link to SQL query from SMC database
sqisql <- read_csv("https://smcchecker.sccwrp.org/smc/sqi_rawdata") %>% 
  
  # group by and retain ID columns
  group_by(masterid, yr, cnty, watershed, regional_board, latitude, longitude, comid) %>% 
  
  # calculate the means of all indices measured multiple times
  summarise(across(d_asci:ps, mean)) %>% 
  
  # ALWAYS ungroup after grouping
  ungroup()
  
# need to add remaining necessary columns using the live-queried dataset
sqidat <- sqisql %>%
  ...
  # predict pChem and pHab using new glms
  # these models are found in R/model_updated.R
  # models were recalibrated in January 2021
  mutate(pChem = predict(wqglm, newdata = sqisql, type = "response"),
     pHab = predict(habglm, newdata = sqisql, type = "response")) %>%
  ...
  mutate(BiologicalCondition = case_when(
    ...
    CSCI_rc == "la" | CSCI_rc == "vla" &
      ASCI_rc == "la" | ASCI_rc == "vla" ~ "Impacted for CSCI and ASCI")) %>%
  ...
  mutate(OverallStressCondition_detail = case_when(
    ...
    WaterChemistryCondition == "Severe" ~ "Stressed by chemistry degradation", 
    HabitatCondition == "Severe" ~ "Stressed by habitat degradation", 
    WaterChemistryCondition == "Severe" &
      HabitatCondition == "Severe" ~ "Stressed by chemistry and habitat degradation", 
    WaterChemistryCondition == "Low" | WaterChemistryCondition == "Moderate" & 
      HabitatCondition == "Low" | HabitatCondition == "Moderate" &
      OverallStressCondition == "Severe" ~ "Stressed by low levels of chemistry or habitat degradation")) %>%
  
  # add Stream Health Index column (based on table in SQI Shiny application & Beck et al., 2019, Section 2.4)
  ...
  # rename column names to agree with remaining scripts/code
  rename(MasterID = masterid,
    SMC_Name = watershed,
    RBNAME = regional_board,
    ASCI = d_asci,
    CSCI = csci,
    IPI = ipi,
    XCMG = xcmg,
    PCT_SAFN = pct_safn,
    Ev_FlowHab = ev_flowhab,
    H_AqHab = h_aqhab,
    H_SubNat = h_subnat,
    TN = tn,
    TP = tp,
  ...
  left_join(cnstr %>% st_set_geometry(NULL), by = "COMID")
...
This work is licensed under the Creative Commons [CC0](https://tldrlegal.com/license/creative-commons-cc0-1.0-universal). These are preliminary results, and are subject to change. For questions, contact [Raphael Mazor](mailto:raphaelm@sccwrp.org) or [Ken Schiff](mailto:kens@sccwrp.org).  Please visit [here](https://github.com/SCCWRP/SQI_Shiny) to view the web page source content. 
