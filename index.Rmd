...
polys <- reactive({
  ...
  if(flby == 'Constraint classes'){ # use sheds as bas
    out <- sheds %>% 
      rename(pllb = SMC_Name)
    return(out)
  }
...
column(width = 12, 
  ...
  column(width = 6, 
    ...
    selectInput('flby', 'Filter by:', choices = c('SMC watersheds', 'Counties', 'Regional boards', 'Constraint classes'))
