...
The SQI can describe stream quality relative to an overall score, the biological condition, the stress condition, and details about the stress condition.  This information can be viewed on the **Scores** tab and on the map using the dropdown menu:

```{r, out.width = '35%', fig.align = 'center'}
knitr::include_graphics('www/map_options.png')
```

The overall SQI score for a site (far right on the above figure) is based on the categorical combinations of stressor (unstressed, stressed) and biological (healthy, impacted) conditions.  Based on these combinations, a site is assigned one of four categorial scores shown below.  
