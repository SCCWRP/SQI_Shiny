...
numlab <- c('CSCI', 'ASCI', 'Total nitrogen', 'Total phosphorus', 'Conductivity', 'CRAM index score', '% sands and fines', 'SW diversity of aquatic habitat', 'SW diversity of streambed substrate', 'Evenness of flow habitat types', 'Riparian veg. cover')
names(numnms) <- numlab
...
output$plodsthab <- renderPlot({
  ...
  # filter biodata
  selvr <-  c('CRAM index score', '% sands and fines', 'SW diversity of aquatic habitat', 'SW diversity of streambed substrate', 'Evenness of flow habitat types', 'Riparian veg. cover')
  toplodst <- dstdat %>% 
    filter(var %in% selvr)
  toplocats <- catslng %>% 
    filter(var %in% selvr)
