...
# gauge colors for boxplots
ggecol <- c('#ff0000', '#f9c802', '#a9d70b')
      ...
      plos = purrr::pmap(list(as.character(var), data, lims, val), function(var, data, lims, val){
        ...
        toplo <- data %>%
          mutate(
            val = val,
            catsval = val,
            catsval = cut(catsval, breaks = lims, labels = c('lo', 'md', 'hi'))
            )
        
        # boxplot
        p <- ggplot(toplo, aes(x = factor(var), y = distval)) + 
          geom_boxplot() + 
          geom_hline(aes(yintercept = val, colour = catsval), size = 1) +
          theme_bw() + 
          theme(
          ...
          xlab(var) + 
          scale_colour_manual(values = rev(ggecol))
...
# column(width = 12, 
# shinydashboard::box(
  renderPlot({plodstbio()$plos[[1]]}, width = 200) #, width = 12
#   )
# )
```
